...
public class VideoPlayerRecyclerView extends RecyclerView {
    ...
    public void playVideo(boolean isEndOfList) {
        ...
        if (mediaUrl != null && mediaObjects.get(targetPosition).getMediaType()==3)
        {
        ...
        else if(mediaUrl!=null && mediaObjects.get(targetPosition).getMediaType()==1) {
            videoSurfaceView.setVisibility(GONE);
            thumbnail.setVisibility(VISIBLE);
            Glide.with(context).load(mediaObjects.get(targetPosition).getMediaURL()).into(holder.thumbnail);
        }
        MediaObject dataModel = mediaObjects.get(targetPosition);
        if(mediaObjects.get(targetPosition).getPostedByName()!=null)
        {
            holder.title.setText(dataModel.getPostedByName());
        }
        if(dataModel.getPostedByCollege()!=null && dataModel.getPostedByCollege().length()>0 )
        {
            holder.title.setText(dataModel.getPostedByName()+" "+ dataModel.getPostedByCollege());
        }
    }
